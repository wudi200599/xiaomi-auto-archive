...
|---|---|---|---|
| su7-2024-standard | su7-2024-max | 3 | 银色影棚 |
...
完整来源 URL 见 `data-raw/official-images/manifest.json`（含每张图的原始地址）。
...
`tools/build-car-images.mjs` 顶部的 `PICKS` 是唯一的选图表；改图只需改这里再重跑。
...
4. 初代 SU7（2024）官网素材已下线，只能走汽车之家或媒体图。
